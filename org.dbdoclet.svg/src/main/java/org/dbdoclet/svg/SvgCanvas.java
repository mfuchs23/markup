...
public abstract class SvgCanvas {
	...
	public void drawImage() {
		...
		computeImageSize(imageWidth, imageHeight);
	...
	private void computeImageSize(int imageWidth, int imageHeight) {
		
		double widthFactor = 1.0;
		
		if (maxWidth > 0) {
			widthFactor = (double) maxWidth / (double) imageWidth;
		}
	
		double heightFactor = 1.0;
		
		if (maxHeight > 0) {
			heightFactor = (double) maxHeight / (double) imageHeight;
		}
		
		if (widthFactor <= heightFactor) {
			maxHeight = (int) (imageHeight * widthFactor);
		} else {
			maxWidth = (int) (imageWidth * heightFactor);
		}
	...
	private Point calculateStartPoint(Connector connector, Cell from, Cell to,
		...
		if (anchor == Connector.ANCHOR_NORTH) {
			...
			ypos1 += (rowInfos[fromRow].getHeight() - fromShape.getHeight())
					/ 2 + fromShape.getTopDistance(xpos1);
		}
