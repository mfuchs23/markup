...
public class XmlText extends XmlElement {
	...
	public void setFont(Font font) {
		...
		if (font == null) {
			throw new IllegalArgumentException(
					"The argument font must not be null!");
		}
		 ...
		 * 
		 * if (font.isBold()) { text.setAttributeNS(null, "font-weight",
		 * "bold"); }
		 * 
		 * if (font.isItalic()) { text.setAttributeNS(null, "font-style",
		 * "italic"); }
		 */
	}
	...
	public void setFill(Color fillColor) {
		text.setAttributeNS(null, "fill",
				SvgServices.getColorAsHexString(fillColor));
	}
